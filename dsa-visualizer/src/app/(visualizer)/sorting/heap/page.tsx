...
export default function HeapSortPage() {
  ...
  useEffect(() => {
    ...
    if (data.length > 0) {
      const generatedSteps = generateHeapSortSteps(data);
    ...
    <div className="flex flex-col h-full w-full max-w-5xl mx-auto p-4 gap-4">
      <div className="flex flex-col gap-2">
         <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-display font-bold text-text-primary">Heap Sort</h1>
            <p className="text-text-secondary mt-1">
              Transforms the array into a max heap, then repeatedly extracts the maximum to build the sorted array.
            </p>
          </div>

...
export default function RadixSortPage() {
  ...
  const generateData = () => {
    ...
    const newArr = Array.from({ length: size }, () => Math.floor(Math.random() * 999)); // High variance so digits differ
    setData(newArr);
    resetVisualizer();
  };
  ...
  useEffect(() => {
    ...
    if (data.length > 0) {
      const generatedSteps = generateRadixSortSteps(data);
    ...
    <div className="flex flex-col h-full w-full max-w-5xl mx-auto p-4 gap-4">
      <div className="flex flex-col gap-2">
         <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-display font-bold text-text-primary">Radix Sort</h1>
      ...
      <div className="bg-bg-card border border-border rounded-lg p-4">
        ...
        {/* Render auxiliary info */}
         {currentStepData?.auxiliaryState?.exp && (
           <div className="mt-2 text-xs font-mono bg-bg-secondary p-2 rounded text-accent-secondary flex gap-4">
             <span>Base 10 Multiplier (Exp): {currentStepData.auxiliaryState.exp}</span>
             {currentStepData.auxiliaryState.currentDigit !== undefined && (
                 <span>Current Mapped Digit: {currentStepData.auxiliaryState.currentDigit}</span>
             )}
           </div>
         )}
      </div>

...
export default function CountingSortPage() {
  ...
  const generateData = () => {
    ...
    const newArr = Array.from({ length: size }, () => Math.floor(Math.random() * 20)); // Small range for visual counting
    setData(newArr);
    resetVisualizer();
  };
  ...
  useEffect(() => {
    ...
    if (data.length > 0) {
      const generatedSteps = generateCountingSortSteps(data);
    ...
    <div className="flex flex-col h-full w-full max-w-5xl mx-auto p-4 gap-4">
      <div className="flex flex-col gap-2">
         <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-display font-bold text-text-primary">Counting Sort</h1>
            <p className="text-text-secondary mt-1">
              An integer sorting algorithm estimating position by counting occurrences of each unique value.
            </p>
          </div>
      ...
      <div className="bg-bg-card border border-border rounded-lg p-4">
        ...
        {/* Render the auxiliary frequency map output from the logic */}
         {currentStepData?.auxiliaryState?.frequencyMap && (
           <div className="mt-2 text-xs font-mono bg-bg-secondary p-2 rounded text-accent-secondary overflow-x-auto">
             Frequencies: {currentStepData.auxiliaryState.frequencyMap}
           </div>
         )}
         {currentStepData?.auxiliaryState?.cumulativeSums && (
           <div className="mt-2 text-xs font-mono bg-bg-secondary p-2 rounded text-indigo-400 overflow-x-auto">
             Cumulative Sums: {currentStepData.auxiliaryState.cumulativeSums}
           </div>
         )}
      </div>
